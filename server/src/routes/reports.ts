...
// ROP/OWNER submits report for a managed user
router.post('/for-user', authenticate, requireRole('OWNER', 'ROP'), async (req: AuthRequest, res: Response) => {
  const { userId, date, type, data, comment } = req.body
  if (!userId || !date || !type || !data) return res.status(400).json({ error: 'Missing fields' })

  try {
    const target = await prisma.user.findFirst({ where: { id: userId, companyId: req.user!.companyId } })
    ...
    const report = await prisma.report.upsert({
      where: { date_userId_type: { date: reportDate, userId, type } },
      update: { data, comment },
      create: { date: reportDate, userId, type, data, comment, departmentId: target.departmentId },
    })
    res.json(report)
  } catch (e) {
    console.error(e)
...
// Get all company reports for a period (OWNER/ROP)
router.get('/company', authenticate, requireRole('OWNER', 'ROP'), async (req: AuthRequest, res: Response) => {
  const { from, to } = req.query
  try {
    const where: any = { user: { companyId: req.user!.companyId } }
    if (from || to) {
      where.date = {}
      if (from) where.date.gte = new Date(from as string)
      if (to) where.date.lte = new Date(to as string)
    ...
    const reports = await prisma.report.findMany({
      where,
      include: { user: { select: { id: true, name: true, role: true, managerType: true, departmentId: true } } },
      orderBy: { date: 'asc' },
    })
    res.json(reports)
  } catch (e) {
