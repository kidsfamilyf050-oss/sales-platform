...
function AddGatewayForm({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="card border-blue-200 bg-blue-50/30 space-y-3">
      <h3 className="text-sm font-semibold text-gray-800">Новый платёжный шлюз</h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <div>
          <label className="label">Внутренний код (value)</label>
          <input
          ...
          />
          <p className="text-xs text-gray-400 mt-0.5">Латиница/цифры/underscore. Не менять после сохранения.</p>
        </div>
        <div>
      ...
      {createMut.isError && (
        <p className="text-xs text-red-600">{(createMut.error as any)?.response?.data?.error || 'Ошибка'}</p>
      )}
      ...
      {editing ? (
        <div className="space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="label text-xs">Комиссия (%)</label>
              <input
                type="number"
                className="input text-sm"
                min={0}
          ...
          <div className="text-xs text-gray-400">Код: <span className="font-mono">{gw.value}</span></div>
  ...
  return (
    <div className="space-y-5 px-4 md:px-6 py-2 md:py-4 max-w-2xl">
      <div className="flex items-start justify-between gap-2">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Платёжные шлюзы</h1>
          <p className="text-sm text-gray-400 mt-0.5">Управление шлюзами и комиссиями для клоузеров</p>
        </div>
        <button
      ...
      {isLoading && <div className="card text-center text-gray-400 py-10">Загрузка...</div>}

      {!isLoading && gateways.length === 0 && (
        <div className="card text-center py-14">
          <CreditCard className="w-10 h-10 text-gray-200 mx-auto mb-3" />
          <p className="text-gray-400 font-medium">Нет шлюзов</p>
        </div>
      )}
      ...
      {active.length > 0 && (
        <div className="space-y-2">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Активные ({active.length})</p>
